...
import fs from 'fs'
import { CommentArr, User, Comment, Reply, ReplyArr } from './type/type';
...
app.post('/register', async (req, res) => {
  ...
  let fileSavedPath: string; // 记录文件的路径
...
app.get('/getComments', async (req, res) => {
  try {
    ...
    const usersDB = new JsonDB(new Config("./data/json/users.json", true, true, '/'))
    let comments: CommentArr = await commentDB.getData("/comments")
    ...
    comments = await Promise.all(comments.map(async (comment): Promise<Comment> => {
